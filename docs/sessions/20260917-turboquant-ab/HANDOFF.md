...
10. **Never LF-normalize with a list comprehension of the form** `[open(p,'wb').write(open(p,'rb').read().replace(b'\r\n', b'\n')) for p in ps]`. Python evaluates `open(p, 'wb')` -- which truncates -- *before* the read expression, so each file is zeroed and then written back empty. This silently emptied two scripts and five result JSONs here, all of which were committed and pushed before anyone noticed. Read into a variable first (`b = open(p,'rb').read()`), then write, and assert the data is non-empty. A 0-byte file whose committed blob is also 0 bytes is the tell; `git cat-file -s <rev>:<path>` confirms it.
11. Backgrounded children of the agent are killed when the session reloads (observed as a KeyboardInterrupt inside a hook mid-run). For long measurements use a detached `schtasks` task, as the 2-bit ladder point and the 8K packed-cache runs did.
